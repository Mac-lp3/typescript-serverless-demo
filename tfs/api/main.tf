...
resource "aws_api_gateway_resource" "slapi_gateway_drugs_proxy" {
  rest_api_id = aws_api_gateway_rest_api.slapi_gateway.id
  parent_id   = aws_api_gateway_rest_api.slapi_gateway.root_resource_id
  path_part   = "drugs/{proxy+}"
}

resource "aws_api_gateway_method" "slapi_gateway_drugs_proxy" {
  rest_api_id   = aws_api_gateway_rest_api.slapi_gateway.id
  resource_id   = aws_api_gateway_resource.slapi_gateway_drugs_proxy.id
  http_method   = "ANY"
  authorization = "NONE"
}
...
resource "aws_api_gateway_integration" "get_drugs_lambda" {
  rest_api_id = aws_api_gateway_rest_api.slapi_gateway.id
  resource_id = aws_api_gateway_method.slapi_gateway_drugs_proxy.resource_id
  http_method = aws_api_gateway_method.slapi_gateway_drugs_proxy.http_method
